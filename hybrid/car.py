...
class CAR:
    ...
    def draw_movingcar(self, moving_vehicles, del_t=0):
        ...
        '''
        new_moving_vehicles, p, = [], []

        for moving_vehicle in moving_vehicles:
            if moving_vehicle[2] == 'h':
                x1 = moving_vehicle[0] - self.l / 2
                y1 = moving_vehicle[1]
                x2 = moving_vehicle[0] + self.l / 2
                y2 = moving_vehicle[1]
                draw_x0 = [x1, x2]
                draw_y0 = [y1, y2]
                p, = plt.plot(draw_x0, draw_y0, c='green', linewidth=10)
                
                coop_x = moving_vehicle[0] + moving_vehicle[3] * del_t
                coop_y = moving_vehicle[1]
                new_moving_vehicles.append([coop_x, coop_y, 'h', moving_vehicle[3]])
            else:
                x1 = moving_vehicle[0]
                y1 = moving_vehicle[1] - self.l / 2
                x2 = moving_vehicle[0]
                y2 = moving_vehicle[1] + self.l / 2
                draw_x0 = [x1, x2]
                draw_y0 = [y1, y2]
                p, = plt.plot(draw_x0, draw_y0, c='green', linewidth=10)
                
                coop_x = moving_vehicle[0]
                coop_y = moving_vehicle[1] + moving_vehicle[3] * del_t
                new_moving_vehicles.append([coop_x, coop_y, 'v', moving_vehicle[3]])
