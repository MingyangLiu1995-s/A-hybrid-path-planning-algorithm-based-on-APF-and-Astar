...
class APF:
    ...
    def keeplane(self, front_p, supply, iters):
        use_sollinesx, use_sollinesy = supply.useful_lines(front_p)

        if use_sollinesx != [] and use_sollinesy != []:  # 直角路口
            F_lane = supply.lane_xy(front_p, use_sollinesx, use_sollinesy, iters)
        elif use_sollinesx != [] and use_sollinesy == []:  # 水平路或者水平丁字路口
            F_lane = supply.lane_x(front_p, use_sollinesx, iters)
        elif use_sollinesy != [] and use_sollinesx == []:  # 竖直路或者竖直丁字路口
            F_lane = supply.lane_y(front_p, use_sollinesy, iters)
        else:  # 十字路口
            F_lane = supply.lane(front_p, iters)

        return F_lane, use_sollinesx, use_sollinesy

    def repulsion(self, front_p, emergency, use_sollinesx, use_sollinesy, iters):
        F_obsx, F_obsy = 0, 0
    ...
    def leavecar(self, front_p, moving_vehicles, emergency, use_sollinesx, use_sollinesy, iters):
        F_vehx, F_vehy = 0, 0
        car_Le = self.L
        car_Be = 0.5 * self.B
    ...
    def pathplanning(self, rear_p, front_p, fi, target, iters, moving_vehicles):
        ...
        while distance > self.target_area and iters <= self.max_iters:
            ...
            F_lane, use_sollinesx, use_sollinesy = self.keeplane(front_p, supply, iters)
            self.F_lanes.append(F_lane)
            F_obs = self.repulsion(front_p, emergency, use_sollinesx, use_sollinesy, iters)
            self.F_obss.append(F_obs)
            F_veh = self.leavecar(front_p, moving_vehicles, emergency, use_sollinesx, use_sollinesy, iters)
