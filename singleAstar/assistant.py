...
class MAP:
    ...
    def __init__(self, length, width, lane_width, L, B):
        self.length = length
        self.width = width
        self.lw = lane_width
        self.L = L
    ...
    def create_lineareas(self, solid_lines):
        line_areas = []

        for solid_line in solid_lines:
            if solid_line[3] == 'h':
                line_areas.append([solid_line[0] - 2.5, solid_line[2] - self.lw / 3,
                                   solid_line[1] + 2.5, solid_line[2] + self.lw / 3])
            else:
                line_areas.append([solid_line[0] - self.lw / 3, solid_line[1] - 2.5,
                                   solid_line[0] + self.lw / 3, solid_line[2] + 2.5])
    ...
    def get_obsareas(self, obstacles):
        obstacle_areas = []

        for obstacle in obstacles:
            obs_Le = 1.1 * obstacle[3] * obstacle[2]
            obs_Be = 0.7 * obstacle[3] * self.B
            if obstacle[4] == 'h':
                obstacle_areas.append([obstacle[0] - obs_Le, obstacle[1] - obs_Be,
                                       obstacle[0] + obs_Le, obstacle[1] + obs_Be])
            else:
                obstacle_areas.append([obstacle[0] - obs_Be, obstacle[1] - obs_Le,
                                       obstacle[0] + obs_Be, obstacle[1] + obs_Le])

        return obstacle_areas

    def draw_vehicle(self, fig, vehicles):
        for vehicle in vehicles:
            if vehicle[2] == 'h':
                x1 = vehicle[0] - self.L / 2
                y1 = vehicle[1] - self.B / 2
                rec = Rectangle(xy=(x1, y1), width=self.L, height=self.B, color='blue')
                fig.add_patch(rec)
            else:
                x1 = vehicle[0] - self.B / 2
                y1 = vehicle[1] - self.L / 2
                rec = Rectangle(xy=(x1, y1), width=self.B, height=self.L, color='blue')
                fig.add_patch(rec)

        return

    def get_vehicles(self, fig):
        ...
        self.draw_vehicle(fig, vehicles)

        return vehicles, init_movingvehis, moving_vehicles

    def get_vehareas(self, vehicles, moving_vehicles):
        vehicle_areas, moving_areas = [], []
        car_Le = 1.2 * self.L
        car_Be = 1.2 * self.B

        for vehicle in vehicles:
            if vehicle[2] == 'h':
                vehicle_areas.append([vehicle[0] - car_Le, vehicle[1] - car_Be,
                                      vehicle[0] + car_Le, vehicle[1] + car_Be])
            else:
                vehicle_areas.append([vehicle[0] - car_Be, vehicle[1] - car_Le,
        ...
        return vehicle_areas, moving_areas
